...
/*--------------------------------------------------------------*/
 ...
 function updateLastLogIn($user_id)
  ...
  /*--------------------------------------------------------------*/
  function veri_inv($s_qty,$p_id){
    global $db;
    $id = (int) $p_id;
    $sql = "SELECT quantity FROM products WHERE id = '$p_id'";
    $result = $db->query($sql);
    $row = mysqli_fetch_array($result);
    return($s_qty > $row['quantity'] ? true : false);
  }
 ...
 function find_all_sale(){
   global $db;
   $sql  = "SELECT s.id,s.qty,s.price,s.date,p.name, s.NameC";
   $sql .= " FROM sales s";
...
function find_sale_by_dates($start_date,$end_date){
  ...
  $sql  = "SELECT s.date, p.name,p.sale_price,p.buy_price, s.NameC,";
  ...
  $sql .= " GROUP BY DATE(s.date),p.name, s.NameC";
...
function  dailySales($year,$month){
  global $db;
  $sql  = "SELECT s.qty,";
  $sql .= " DATE_FORMAT(s.date, '%Y-%m-%e') AS date,p.name, s.NameC,";
  $sql .= "SUM(p.sale_price * s.qty) AS total_saleing_price";
  $sql .= " FROM sales s";
  $sql .= " LEFT JOIN products p ON s.product_id = p.id";
