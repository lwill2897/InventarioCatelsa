...
if(!$sale){
  $session->msg("d","Falta el ID de producto.");
  redirect('sales.php');
}
?>
...
<?php
  ...
  if(isset($_POST['update_sale'])){
    $req_fields = array('title','NameClient','quantity','price','total', 'date' );
    validate_fields($req_fields);
        if(empty($errors)){
          $p_id      = $db->escape((int)$product['id']);
          $p_nameC   = $db->escape($_POST['NameClient']);
          ...
          $sql .= " product_id= '{$p_id}',NameC='{$p_nameC}',qty={$s_qty},price='{$s_total}',date='{$s_date}'";
          $sql .= " WHERE id ='{$sale['id']}'";
          $result = $db->query($sql);
          if( $result && $db->affected_rows() === 1){
                    update_product_qty($p_nameC,$s_qty,$p_id);
                    $session->msg('s',"Venta Actualizada.");
                    redirect('edit_sale.php?id='.$sale['id'], false);
                  } else {
                    $session->msg('d',' ¡Lo sentimos, no se pudo actualizar!!');
          ...
          <th> Acción </th>
           ...
           <tbody  id="product_info">
              ...
              <form method="post" action="edit_sale.php?id=<?php echo (int)$sale['id']; ?>">
                ...
                <td id="p_nameC">
                  <input type="text" class="form-control" id="sug_input" name="NameClient" value="<?php echo remove_junk($sale['NameC']); ?>">
                  <div id="result" class="list-group"></div>
                </td>
                <td id="s_qty">
